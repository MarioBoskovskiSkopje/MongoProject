...
module.exports = {
    ...
    show: function (req, res) {
        ...
        FlavourMixOrdersModel.findOne({_id: id}, function (err, FlavourMixOrders) {
            if (err) {
                return res.status(500).json({
                    message: 'Error when getting FlavourMixOrders.',
                    error: err
    ...
    update: function (req, res) {
        ...
        FlavourMixOrdersModel.findOne({_id: id}, function (err, FlavourMixOrders) {
            ...
            FlavourMixOrders.batch = req.body.batch ? req.body.batch : FlavourMixOrders.batch;
			FlavourMixOrders.final_status = req.body.final_status ? req.body.final_status : FlavourMixOrders.final_status;
			FlavourMixOrders.flavourmix = req.body.flavourmix ? req.body.flavourmix : FlavourMixOrders.flavourmix;
			FlavourMixOrders.movedtoNext = req.body.movedtoNext ? req.body.movedtoNext : FlavourMixOrders.movedtoNext;
			FlavourMixOrders.orderdate = req.body.orderdate ? req.body.orderdate : FlavourMixOrders.orderdate;
			FlavourMixOrders.row = req.body.row ? req.body.row : FlavourMixOrders.row;
			FlavourMixOrders.stocking = req.body.stocking ? req.body.stocking : FlavourMixOrders.stocking;
    ...
    remove: function (req, res) {
        var id = req.params.id;
        FlavourMixOrdersModel.findByIdAndRemove(id, function (err, FlavourMixOrders) {
            if (err) {
                return res.status(500).json({
